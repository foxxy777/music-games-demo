...
| T1 | 键盘渲染 | 12 键（含升号）、Do→Si 顺序、音名 4C~4B、斯克里亚宾色相变量 |
| T2 | 音频资产 | 24 个 WAV 全部 200 + RIFF/WAVE 头 + 大小合理（防 08-24 那种错素材混入的结构性回归） |
...
| T7 | 布局不变量 | 舞台 16:9、键等宽、键排占行宽 ≥95%、图片区 ≥50% 高 |
| T8 | 手机缩放 | 888×444 视口下舞台等比缩放不出界、键 CSS 宽度仍可点（>40px） |
| T9 | 回放防叠加 | 回放中再点=停止；点琴键=打断（08-26 修复项） |
| T10 | 听题中点键 | 停一切转作答、半答保留（08-26 拍板） |
| T11 | 连点单声部 | 新点键音掐旧点键音（08-26 拍板） |
| T12 | 谱面渲染 | 五线谱两行谱表+G谱号，14 音 = 题库#89 小星星，二分/下加线/歌词/播放按钮（08-30） |
| T13 | 跟谱联动 | 播放→逐音高亮+底键同步亮；再点=停止清高亮；点琴键=打断（08-30） |
